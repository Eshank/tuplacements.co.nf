...
if(isset($_GET['p']))
{
echo '<form method="post" action="quizp.php';
echo '?p='.$_GET['p'];
echo '">';
echo '<div style="padding:30px;margin:30px;">';
	...
	if($_GET['p']==1)
		...
		$sql = "select * from cpp where id between 26 and 50";
	else if($_GET['p']==3)
		$sql = "select * from cpp where id between 51 and 75";
	else if($_GET['p']==4)
		$sql = "select * from cpp where id between 76 and 100";
	else if($_GET['p']==5)
		$sql = "select * from cpp where id between 101 and 125";
	else if($_GET['p']==6)
		$sql = "select * from cpp where id between 126 and 150";
	else if($_GET['p']==7)
		$sql = "select * from cpp where id between 151 and 175";
	$query = mysqli_query($conn, $sql);
	while($row = mysqli_fetch_array($query))
	{
		echo '<div style="color:#2c3e50;font-size:20px;font-weight:bold;">QUES NO '.$i.''.$row['question'].'</div> ';
		$sql2 = "select * from cpp where id=".$row['id'];
    ...
    if(isset($_POST['submit']))
		{
			if(isset($_POST[$i]))
			{
				if($_POST[$i]=='A')
				{
					$sql3 = "select * from cpp where id=".$row['id'];
